...
def main():
    ...
    df['open_trade'] = np.where(open_condition, df['BuyPrice'], '')
    df['close_trade'] = np.where(close_condition, df['SellPrice'].astype(str)+str(' Close'), '')
    df['raw_trade'] = df['open_trade'] + df['close_trade']
    df['raw_trade'] = df['raw_trade'].replace('', np.nan)
    test1 = df['raw_trade'].dropna()
    print(test1)
